...
export default function Profile() {
  ...
  const [activeTab, setActiveTab] = useState<"personal" | "upgrades" | "payment">("personal");

  useEffect(() => {
    const fetchUser = async () => {
      try {
        const response = await fetch("https://api.mutifacebook.pro.vn/auth/me", {
          headers: { Authorization: `Bearer ${localStorage.getItem("authToken")}` }, // Sửa key thành authToken
        });
        ...
        setError(apiError.message || "Lỗi không xác định");
        setLoading(false);
      }
    };
    fetchUser();
              ...
              <ul className="space-y-2">
                <li>
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 rounded-md ${activeTab === "personal" ? "bg-teal-700" : "hover:bg-teal-700"
                ...
                <li>
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 rounded-md ${activeTab === "upgrades" ? "bg-teal-700" : "hover:bg-teal-700"
                      } transition duration-200`}
                  >
                    Các gói nâng cấp
                  </button>
                ...
                <li>
                  <button
                    ...
                    className={`w-full text-left px-4 py-2 rounded-md ${activeTab === "payment" ? "bg-teal-700" : "hover:bg-teal-700"
                      } transition duration-200`}
                  >
